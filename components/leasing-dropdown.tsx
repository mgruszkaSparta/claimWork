...
import type { LeasingCompany, LeasingCompanySelectionEvent } from "@/types/leasing"
import { LeasingCompaniesService } from "@/lib/leasing-companies"

interface LeasingDropdownProps {
  selectedCompanyId?: number
  onCompanySelected?: (event: LeasingCompanySelectionEvent) => void
  className?: string
}
...
export default function LeasingDropdown({
  selectedCompanyId,
  onCompanySelected,
  className = "",
}: LeasingDropdownProps) {
  const [companies] = useState<LeasingCompany[]>(
    LeasingCompaniesService.sortCompaniesAlphabetically(LeasingCompaniesService.getCompanies()),
  )
  const [filteredCompanies, setFilteredCompanies] = useState<LeasingCompany[]>(companies)
  ...
  useEffect(() => {
    if (selectedCompanyId) {
      const company = LeasingCompaniesService.getCompanyById(selectedCompanyId)
      if (company) {
  ...
  const selectCompany = (company: LeasingCompany) => {
    setSelectedCompany(company)
    setIsDropdownOpen(false)
    setSearchTerm("")
  ...
  return (
    <div className={`leasing-dropdown relative w-full ${className}`}>
      {/* Dropdown Button */}
      <Button
        ref={buttonRef}
        variant="outline"
      ...
      >
        <span className="truncate text-gray-900">
          {selectedCompany ? selectedCompany.name : "Wybierz firmę leasingową..."}
        </span>
